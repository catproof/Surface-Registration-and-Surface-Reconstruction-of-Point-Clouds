...
#edge case where the binary search ends up in a spot with duplicates to the right of the found value
def insert_after_any_duplicates(list_to_search, value_to_add, curr_value_index):
    last_index_of_list_to_seach = len(list_to_search) - 1
    if curr_value_index == last_index_of_list_to_seach:
        list_to_search[curr_value_index] = value_to_add
        return True
        
    if list_to_search[curr_value_index][1] != list_to_search[curr_value_index + 1][1]:
        list_to_search[curr_value_index] = value_to_add
        return True
        
    while curr_value_index < last_index_of_list_to_seach:
        if list_to_search[curr_value_index + 1][1] > list_to_search[curr_value_index][1]:
            curr_value_index = curr_value_index + 1
            break
        curr_value_index = curr_value_index + 1
...
def binary_search_and_add_helper(list_to_search, value_to_add, left_bound, right_bound):
    if left_bound == right_bound:
        return insert_after_any_duplicates(list_to_search, value_to_add, right_bound)
        # list_to_search[right_bound] = value_to_add
        # return True
    if right_bound - left_bound == 1:
        if list_to_search[left_bound][1] == list_to_search[right_bound][1]:
            return insert_after_any_duplicates(list_to_search, value_to_add, left_bound)
            #list_to_search[left_bound] = value_to_add
        elif value_to_add[1] <= list_to_search[right_bound][1] and value_to_add[1] > list_to_search[left_bound][1]:
            return insert_after_any_duplicates(list_to_search, value_to_add, right_bound)
            #list_to_search[right_bound] = value_to_add
        elif value_to_add[1] <= list_to_search[left_bound][1]:
            return insert_after_any_duplicates(list_to_search, value_to_add, left_bound)
            #list_to_search[left_bound] = value_to_add
        else:
            print('unknown error inserting value into list')
            return False
        return True
    
    curr_value_index = left_bound + round((right_bound - left_bound)/2)
    curr_value = list_to_search[curr_value_index][1]
    if curr_value == value_to_add[1]:
        return insert_after_any_duplicates(list_to_search, value_to_add, curr_value_index)
        # list_to_search[curr_value_index] = value_to_add
        # return True
    if curr_value < value_to_add[1]:
        return binary_search_and_add_helper(list_to_search, value_to_add, curr_value_index, right_bound)
    if curr_value > value_to_add[1]:
        return binary_search_and_add_helper(list_to_search, value_to_add, left_bound, curr_value_index)
    return False

def binary_search_and_add(list_to_search, value_to_add, left_bound = 0, right_bound = None):
    if right_bound == None:
        right_bound = len(list_to_search) - 1
    return binary_search_and_add_helper(list_to_search, value_to_add, left_bound, right_bound)
    
def best_matches(desc1, desc2, num_matches):
    """Find the best matches for two sets of descriptors"""

    if num_matches > desc2.shape[0]:
        print("num_matches must be less than the number of descriptors in desc2")
        return []
    ...
    for i in range(desc1.shape[0]):
        #min_distances = np.full((num_matches,2), float('inf'))
        min_distances = SortedList(key=lambda l:l[1])
        for j in range(num_matches):
            # Compute Euclidean distance
            dist = np.linalg.norm(desc1[i, :] - desc2[j, :])       
            min_distances.add([j,dist])
        for j in range(num_matches, desc2.shape[0]):
            # Compute Euclidean distance
            dist = np.linalg.norm(desc1[i, :] - desc2[j, :])            
            if dist < min_distances[-1][1]:
                min_distances.pop()
                min_distances.add([j,dist])
                
            # if len(min_distances) < num_matches:
                # min_distances.append([j, dist])
                # if len(min_distances) == num_matches:
                    # min_distances = sorted(min_distances,key=lambda l:l[1])
            # elif dist < min_distances[-1][1]:
                # binary_search_and_add(min_distances, [j, dist])
        
        corr.append([i, min_distances])
